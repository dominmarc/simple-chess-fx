...
public class StartingController implements IController {
    ...
    public void connectButtonClicked() {
        ...
        // 01 means client
        String message = "01" + port;

        // start
        try {
            open(FileProvider.get().getFile(Constants.LOCAL_NETWORK_GAME_FILE), null, FileProvider.get().getFile(Constants.GAME_STYLE), message);
        } catch (FileNotFoundException e) {
            PopUpProvider.createInfoPopUp("File not found!").showPopUp();
    ...
    public void loMuButtonClicked() {
        ...
        try {
            open(FileProvider.get().getFile(Constants.LOCAL_GAME_FILE), null, FileProvider.get().getFile(Constants.GAME_STYLE), "");
        } catch (FileNotFoundException e) {
            PopUpProvider.createInfoPopUp("File not found!").showPopUp();
            LOGGER.warn(e.getClass().getSimpleName() + ": " + e.getMessage());
        }
     ...
     */
    private void open(Path fxmlFile, Image icon, Path style, String initialValue) {
        LOGGER.info("Trying to open fxml: [" + fxmlFile + "] with initialValue: " + initialValue + "...");
        FxmlOpener newFXML;

        // construct opener
        try {
            newFXML = new FxmlOpener(fxmlFile, 0, icon, style);
        } catch (Exception e) {
